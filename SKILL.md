...
- Configure the hosted gateway base URL at `https://gw-aql.tomo.services`.
- Configure gateway WebSocket subscriptions at `wss://gw-aql.tomo.services/v1/market/ws`.
...
- If your workflow uses the hosted MCP server at `https://mcp-aql.tomo.services`, keep it separate from the gateway base URL.
...
- Gateway `REST` asset identity reads through `https://gw-aql.tomo.services/v1/market/asset-profile`.
- Gateway `REST` market identity reads through `https://gw-aql.tomo.services/v1/market/market-profile`.
- Gateway `REST` latest price reads through `https://gw-aql.tomo.services/v1/market/price-snapshot`.
- Gateway `REST` short-range OHLCV reads through `https://gw-aql.tomo.services/v1/market/ohlcv-window`.
- Gateway `WebSocket` realtime delivery through `wss://gw-aql.tomo.services/v1/market/ws` for supported live event targets.
- Gateway raw `BSC` and `Solana` compatibility through `https://gw-aql.tomo.services/rpc/bsc` and `https://gw-aql.tomo.services/rpc/solana`.
...
- If the hosted MCP server is explicitly needed, use `https://mcp-aql.tomo.services` for that workflow rather than treating it as a gateway route.
